...
//   }
// }
// call_notification.dart (FIXED - Patient Side)
import 'dart:async';
...
class PatientCallListener {
  ...
  void startListening(String patientId, BuildContext context) {
    ...
    // FIXED: Listen for 'ringing' status instead of 'pending'
    _callSubscription = _firestore
        .collection('call_requests')
        .where('patientId', isEqualTo: patientId)
        .where('status', isEqualTo: 'ringing') // ← CRITICAL FIX
        .snapshots()
        .listen(
          (snapshot) {
            for (var docChange in snapshot.docChanges) {
...
// Patient Incoming Call Dialog with Sound
class PatientIncomingCallDialog extends StatefulWidget {
  final String doctorName;
  final String doctorId;
  final String callId;
            ...
            Padding(
              ...
              child: Column(
                children: [
                  CircleAvatar(
                    radius: 40,
                    backgroundColor: widget.callType == 'video'
                        ? Colors.blue.shade100
                        : Colors.green.shade100,
                    child: Icon(
                      Icons.person,
                      size: 50,
                      color: widget.callType == 'video'
